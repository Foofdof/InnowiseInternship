...
master_url = 'spark://26.6.130.57:7077'
...
df_people = spark.read.option('header', 'true').schema(people_schema).csv(
    './data/top_100_richest.csv'
)

df_country = spark.read.option('header', 'true').schema(country_schema).csv(
    './data/wiki_number_of_billionaires.csv'
)


df_people_filtered = df_people.withColumn('net_worth',
                                        regexp_extract(col('net_worth'), "^\$(\\d+).*$", 1).\
                                        cast('int')).filter(col('net_worth') > 60)

df_people_new = df_people_filtered.withColumn('nationality',
                                        when(col('nationality') == 'United States of America',  'United States').\
